...
interface BuildOptionListOptions {
  /**
   * If `true` (default), only rows inside `GRUPPE_S` scope are considered — `INDIVID_S`
...
/**
 * Build an `OptionList` from a CVT DATEN file by pairing `AUFTRAGSAUSDRUCK_A` predicate
 * rows with the immediately-following `FSW_PSW_WW` rows.
 *
 * The pairing is *strictly* by document order — same as NCSDummy. If no AUFTRAGSAUSDRUCK
 * precedes a given FSW_PSW_WW row, we still emit the (FSW, PSW) entry with an empty
...
export function buildOptionList(
  ...
  /** Last AUFTRAGSAUSDRUCK fragment seen, awaiting an FSW_PSW_WW partner. */
  let pendingPredicate: Uint8Array | undefined;
  /** When true (inside INDIVID_S), skip everything until next GRUPPE_S. */
      ...
      case 'FSW_PSW_WW': {
        ...
        if (fsw === undefined || psw === undefined) {
          throw new OptionListError(
            `FSW_PSW_WW row missing field(s) (fsw=${fsw}, psw=${psw})`,
          );
        }
